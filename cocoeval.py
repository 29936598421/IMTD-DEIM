...
class COCOeval:
    ...
    def summarize(self):
        ...
        # ======= 新增每个类别 AP 和 AP50 输出 =======
        if self.eval and self.params.iouType in ["bbox", "segm", "boundary"]:
            cat_names = [self.cocoGt.cats[catId]['name'] for catId in self.params.catIds]
            s = self.eval["precision"]  # [T, R, K, A, M]
            aind = self.params.areaRngLbl.index("all")
            mind = self.params.maxDets.index(100)
            precision = self.eval["precision"]  # shape: [T, K, A, M]
            recall = self.eval["recall"]  # shape: [T, K, A, M]
            T = s.shape[0]  # IoU阈值个数
            print("                                            Ai缝合怪带你冲顶会顶刊--- CVPR2025 DEIM模型指标---转YOLO模型评价指标！               ")
            for k, catId in enumerate(self.params.catIds):
                # AP: mean over all IoU (0.50:0.95)
                s_cat = s[:, :, k, aind, mind]
                s_cat = s_cat[s_cat > -1]
                ap = np.mean(s_cat) if len(s_cat) > 0 else float('nan')
                # AP50: mean over recall when IoU=0.5
                iouThr = 0.5
                t = np.where(np.isclose(self.params.iouThrs, iouThr))[0][0]
                s_cat_50 = s[t, :, k, aind, mind]
                s_cat_50 = s_cat_50[s_cat_50 > -1]
                ap50 = np.mean(s_cat_50) if len(s_cat_50) > 0 else float('nan')

                # AP75: mean over recall when IoU=0.75
                iouThr = 0.75
                t = np.where(np.isclose(self.params.iouThrs, iouThr))[0][0]
                s_cat_50 = s[t, :, k, aind, mind]
                s_cat_50 = s_cat_50[s_cat_50 > -1]
                ap75 = np.mean(s_cat_50) if len(s_cat_50) > 0 else float('nan')

                # 取所有IoU阈值下的平均 Precision 和 Recall
                precisions_cat = precision[:, k, aind, mind]
                recalls_cat = recall[:, k, aind, mind]

                # 过滤掉 -1（无效值）
                valid_precisions = precisions_cat[precisions_cat > -1]
                valid_recalls = recalls_cat[recalls_cat > -1]

                avg_precision = np.mean(valid_precisions) if len(valid_precisions) > 0 else float('nan')
                avg_recall = np.mean(valid_recalls) if len(valid_recalls) > 0 else float('nan')

                self.print_function(f"Category: {cat_names[k]:<20} Precision = {avg_precision: 5.3f} | Recall = {avg_recall: 5.3f}  |  mAP(0.50) = {ap50:5.3f}  |  mAP(0.75) = {ap75:5.3f}  |  mAP(0.50:0.95) = {ap:5.3f}")
