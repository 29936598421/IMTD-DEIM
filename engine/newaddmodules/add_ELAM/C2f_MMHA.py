...
__all__ = ['C2f_MMHA','C2f_MMHABlock']
...
class Bottleneck_MMHA(nn.Module):
    """Standard bottleneck."""
...
class C2f_MMHA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(Bottleneck_MMHA(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class C2f_MMHABlock(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(MMHABlock(self.c) for _ in range(n))
...
if __name__ == "__main__":
    ...
    output= MMHA (input)
    # 打印输入和输出形状
    print("MMHA_输入张量形状:", input.shape)
    print("MMHA_输出张量形状:", output.shape)
