...
'''
__all__ = ['C3k2_SPConv','C2f_SPConv']
...
class APBottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        self.spconv = SPConv(c1,c2)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        """'forward()' applies the YOLO FPN to input data."""

        return x + self.spconv(x) if self.add else self.spconv(x)
...
if __name__ == "__main__":
    ...
    input = torch.randn(1, 32, 64, 64)  # 创建一个形状为 (1,32,64, 64)
    C3k2_SPConv = C3k2_SPConv(32,32) #二次创新SPConv卷积模块
    output = C3k2_SPConv(input)
