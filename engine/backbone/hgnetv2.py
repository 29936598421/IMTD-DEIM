...
class HG_Block(nn.Module):
    def __init__(
        ...
        for i in range(layer_num):
            ...
            elif addconv in {'StarConv','ODConv','DSConv','IDConv','DCNv2','CondConv2D','WTConv','PSConv','FFConv','RFAConv',
                             'RFCAConv','RFCBAMConv','SCConv','LDConv','DEConv','TBConv','PConv','RepConv','DynamicConv',
                             'GBConv','MSGDConv','FADConv','FDConv','ARConv','wConv2D','MBRConv3','MBRConv5','SFSConv',
                             'PATConv','StripConv','GCConv','CGHalfConv','HLKConv'
                             } | {'WFEConv','GSConv','SPConv','MRFAConv'}: # |后面的是二次创新的模块，方便小伙伴观察
                print("Ai缝合怪-带你冲顶会、顶刊--使用了addconv=", addconv)
        ...
        if agg == 'se':
        ...
        elif agg in {'CBAM','CAA','CDFA','ECA','GCSA','SSA','LocalAttention','CLAttention','SCSA','CASAtt','LWGA',
                     'LRSA','KSFA','EMA','SeaAttention','PolaLinearAttention','LEGA','TSA','FCAttention','ASSA','DualdomainSA',
                     'RSA','GSA','CGAttention','DLKA','CloAttention','BiLevelRoutingAttention','RCA','ShuffleAttn',
            ...
            print("Ai缝合怪-带你冲顶会、顶刊--使用了agg_blocks=", agg)
            self.aggregation = nn.Sequential(
...
class HG_Stage(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
'''
这个实现展示了HGNetV2的完整架构，包括：
    基础构建块(ConvBNAct, LightConvBNAct)
    Stem模块(StemBlock)
    注意力机制(EseModule)
    基本块(HG_Block)和阶段(HG_Stage)
    完整的HGNetV2网络
网络特点：
    支持多种配置(B0-B6)
    可选的可学习仿射变换
